...
execute positioned ^ ^ ^-2 if score @s[tag=dynamic] sherd_id = @e[type=#level:level_markers, tag=sherd_data, limit=1, sort=nearest, distance=..2] sherd_id run scoreboard players set @s completion_check 1
execute positioned ^ ^ ^-2 unless score @s[tag=dynamic] sherd_id = @e[type=#level:level_markers, tag=sherd_data, limit=1, sort=nearest, distance=..2] sherd_id run scoreboard players set @s completion_check 0
execute positioned ^ ^2 ^-2 if score @s[tag=dynamic] sherd_id matches -1 run scoreboard players set @s completion_check -1
